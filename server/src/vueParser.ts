...
export class VueASTParser {
   ...
   * 查找指定位置的元素
   */
  public findElementAtPosition(elements: VueElement[], offset: number): VueElement | undefined {
   ...
   * 查找指定位置的属性
   */
  public findAttributeAtPosition(element: VueElement, offset: number): VueAttribute | undefined {
   ...
   * 检查位置是否在属性名上
   */
  public isOnAttributeName(content: string, attribute: VueAttribute, offset: number): boolean {
   ...
   * 检查位置是否在属性值上
   */
  public isOnAttributeValue(content: string, attribute: VueAttribute, offset: number): boolean {
    const attrText = content.substring(attribute.startOffset, attribute.endOffset)
    const equalIndex = attrText.indexOf('=')
