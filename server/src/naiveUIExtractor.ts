...
export class NaiveUIExtractor {
  ...
  private extractFromComponentsDir(): ComponentMetadata[] {
    ...
    const componentsPath = fs.existsSync(esPath) 
      ? path.join(esPath, 'components')
      : fs.existsSync(libPath)
      ? path.join(libPath, 'components')
      : null;
  ...
  private extractComponentFromDir(basePath: string, dirName: string): ComponentMetadata | null {
    ...
    const propsFile = files.find(f => 
      f.toLowerCase().includes('props') && f.endsWith('.ts')
    );
